...
try:
    # Added after the paper's Transformers 4.41 release. Keep deterministic
    # FlashAttention support when a newer compatible Transformers is inspected,
    # without making that private module a requirement of the paper environment.
    from transformers import modeling_flash_attention_utils as _flash_attention_utils
except ImportError:
    _flash_attention_utils = None
...
class DeepspeedStrategy(ABC):
    ...
    def setup_distributed(self, timeout=timedelta(minutes=60)) -> None:
        if self.full_determinism:
            ...
            if _flash_attention_utils is not None:
                _flash_attention_utils.deterministic_g = True
        else:
    ...
    def setup_dataloader(
        ...
        if sampler is None and dist.is_initialized():
            ...
            sampler = DistributedSampler(
                ...
                drop_last_multiple=drop_last_multiple,
            )

        # The paper stack uses PyTorch 2.3.1. TorchData's StatefulDataLoader was
        # introduced with the Torch 2.4 generation, and this project does not use
        # its state_dict API; resume position is handled by DistributedSampler.
        return DataLoader(
            replay_buffer,
