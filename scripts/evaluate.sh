...
CR="${1:-16}"
EVAL_DATA_PATH="${EVAL_DATA_PATH:?set EVAL_DATA_PATH to the alias-complete aria-data evaluation artifact}"
...
EXTRA_ARGS=(--eval_data_path "${EVAL_DATA_PATH}")
if [[ "${RAG_CONFIGURATION}" == "clara_baseline" ]]; then
  ...
  EXTRA_ARGS+=(--corpus_path "${CORPUS_PATH}" --doc_embeddings "${DOC_EMBEDDINGS}")
fi
...
python -m openrlhf.cli.evaluate_aria \
  ...
  --compression_rate "${CR}" \
  --output_dir "${EVAL_OUTPUT_DIR:-${ROOT_DIR}/eval_results/cr${CR}}" \
  --device "${DEVICE:-cuda}" \
  "${EXTRA_ARGS[@]}"
